...
import com.leowan.pss.query.ProductStockQuery;
...
public class ProductStockAction extends CRUDAction<ProductStock> {
	 ...
	 * productStockService.findByQuery(baseQuery); //
	...
	protected void list() throws Exception {
		pageList = productStockService.findByQuery(baseQuery);
...
//		putContext("allDeps", departmentService.getAll());
	...
	public void prepareSave() throws Exception {
		...
		// 如果id为空,那么就实例化
		if (id == null) {
			this.productStock = new ProductStock();// 压栈
		} else {
			// 如果id不为空,就从数据库里面查询productStock,那么数据就不会丢失
...
//			productStock.setDepartment(null);
	...
	public void prepareInput() throws Exception {
		...
		if (id != null) {
			this.productStock = productStockService.get(id); // 通过获取到的id查询回显.productStock就到栈顶了
