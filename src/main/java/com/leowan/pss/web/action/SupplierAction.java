...
import com.leowan.pss.query.SupplierQuery;
...
public class SupplierAction extends CRUDAction<Supplier> {
	 ...
	 * supplierService.findByQuery(baseQuery); //
	...
	protected void list() throws Exception {
		pageList = supplierService.findByQuery(baseQuery);
...
//		putContext("allDeps", departmentService.getAll());
	...
	public void prepareSave() throws Exception {
		...
		// 如果id为空,那么就实例化
		if (id == null) {
			this.supplier = new Supplier();// 压栈
		} else {
			// 如果id不为空,就从数据库里面查询supplier,那么数据就不会丢失
...
//			supplier.setDepartment(null);
	...
	public void prepareInput() throws Exception {
		...
		if (id != null) {
			this.supplier = supplierService.get(id); // 通过获取到的id查询回显.supplier就到栈顶了
