...
import com.leowan.pss.query.MenuQuery;
...
public class MenuAction extends CRUDAction<Menu> {
	 ...
	 * menuService.findByQuery(baseQuery); //
...
//		putContext("allDeps", departmentService.getAll());
	...
	public void prepareSave() throws Exception {
		...
		// 如果id为空,那么就实例化
		if (id == null) {
			this.menu = new Menu();// 压栈
		} else {
			// 如果id不为空,就从数据库里面查询menu,那么数据就不会丢失
...
//			menu.setDepartment(null);
	...
	public void prepareInput() throws Exception {
		...
		if (id != null) {
			this.menu = menuService.get(id); // 通过获取到的id查询回显.menu就到栈顶了
