...
import com.leowan.pss.query.StockIncomeBillQuery;
...
public class StockIncomeBillAction extends CRUDAction<StockIncomeBill> {
	 ...
	 * stockIncomeBillService.findByQuery(baseQuery); //
	...
	protected void list() throws Exception {
		pageList = stockIncomeBillService.findByQuery(baseQuery);
...
//		putContext("allDeps", departmentService.getAll());
	...
	public void prepareSave() throws Exception {
		...
		// 如果id为空,那么就实例化
		if (id == null) {
			this.stockIncomeBill = new StockIncomeBill();// 压栈
		} else {
			// 如果id不为空,就从数据库里面查询stockIncomeBill,那么数据就不会丢失
			this.stockIncomeBill = stockIncomeBillService.get(id);
			// 处理修改部门名称的问题
			// jpa中不能修改持久状态对象的主键. 需要将其设为null再由spring new出来
//			stockIncomeBill.setDepartment(null);
		}
	}
	...
	public void prepareInput() throws Exception {
		...
		if (id != null) {
			this.stockIncomeBill = stockIncomeBillService.get(id); // 通过获取到的id查询回显.stockIncomeBill就到栈顶了
