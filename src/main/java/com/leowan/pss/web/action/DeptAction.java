...
import com.leowan.pss.query.DeptQuery;
...
public class DeptAction extends CRUDAction<Dept> {
	 ...
	 * deptService.findByQuery(baseQuery); //
	...
	protected void list() throws Exception {
		pageList = deptService.findByQuery(baseQuery);
...
//		putContext("allDeps", departmentService.getAll());
	...
	public void prepareSave() throws Exception {
		...
		// 如果id为空,那么就实例化
		if (id == null) {
			this.dept = new Dept();// 压栈
		} else {
			// 如果id不为空,就从数据库里面查询dept,那么数据就不会丢失
...
//			dept.setDepartment(null);
	...
	public void prepareInput() throws Exception {
		...
		if (id != null) {
			this.dept = deptService.get(id); // 通过获取到的id查询回显.dept就到栈顶了
