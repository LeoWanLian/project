...
import com.leowan.pss.query.DepotQuery;
...
public class DepotAction extends CRUDAction<Depot> {
	 ...
	 * depotService.findByQuery(baseQuery); //
	...
	protected void list() throws Exception {
		pageList = depotService.findByQuery(baseQuery);
...
//		putContext("allDeps", departmentService.getAll());
	...
	public void prepareSave() throws Exception {
		...
		// 如果id为空,那么就实例化
		if (id == null) {
			this.depot = new Depot();// 压栈
		} else {
			// 如果id不为空,就从数据库里面查询depot,那么数据就不会丢失
...
//			depot.setDepartment(null);
	...
	public void prepareInput() throws Exception {
		...
		if (id != null) {
			this.depot = depotService.get(id); // 通过获取到的id查询回显.depot就到栈顶了
