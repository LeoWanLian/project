...
import com.leowan.pss.query.SystemDictionaryTypeQuery;
...
public class SystemDictionaryTypeAction extends CRUDAction<SystemDictionaryType> {
	 ...
	 * systemDictionaryTypeService.findByQuery(baseQuery); //
...
//		putContext("allDeps", departmentService.getAll());
	...
	public void prepareSave() throws Exception {
		...
		// 如果id为空,那么就实例化
		if (id == null) {
			this.systemDictionaryType = new SystemDictionaryType();// 压栈
		} else {
			// 如果id不为空,就从数据库里面查询systemDictionaryType,那么数据就不会丢失
...
//			systemDictionaryType.setDepartment(null);
	...
	public void prepareInput() throws Exception {
		...
		if (id != null) {
			this.systemDictionaryType = systemDictionaryTypeService.get(id); // 通过获取到的id查询回显.systemDictionaryType就到栈顶了
