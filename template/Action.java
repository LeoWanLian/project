...
import com.leowan.pss.query.${entityDomain}Query;
...
public class ${entityDomain}Action extends CRUDAction<${entityDomain}> {
	...
	private ${entityDomain}Query baseQuery = new ${entityDomain}Query();

	private ${entityDomain} ${lowerEntityDomain};

	/*
	 * @Override public String execute() throws Exception {
	 * System.out.println("execute"); pageList =
	 * ${lowerEntityDomain}Service.findByQuery(baseQuery); //
	...
	protected void list() throws Exception {
		pageList = ${lowerEntityDomain}Service.findByQuery(baseQuery);
...
//		putContext("allDeps", departmentService.getAll());
	...
	public void prepareSave() throws Exception {
		...
		// 如果id为空,那么就实例化
		if (id == null) {
			this.${lowerEntityDomain} = new ${entityDomain}();// 压栈
		} else {
			// 如果id不为空,就从数据库里面查询${lowerEntityDomain},那么数据就不会丢失
			this.${lowerEntityDomain} = ${lowerEntityDomain}Service.get(id);
			// 处理修改部门名称的问题
			// jpa中不能修改持久状态对象的主键. 需要将其设为null再由spring new出来
//			${lowerEntityDomain}.setDepartment(null);
		}
	}
	...
	public void prepareInput() throws Exception {
		...
		if (id != null) {
			this.${lowerEntityDomain} = ${lowerEntityDomain}Service.get(id); // 通过获取到的id查询回显.${lowerEntityDomain}就到栈顶了
			// System.out.println(${lowerEntityDomain});
		}
	}

	public ${entityDomain}Query getBaseQuery() {
		return baseQuery;
	}

	public void setBaseQuery(${entityDomain}Query baseQuery) {
